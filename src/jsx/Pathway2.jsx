...
function App() {
  useEffect(() => {
    const vis_container = d3.select('#app-root-2023-sdg_costing_pathway2 .dots');
            ...
            {(isVisible) => (
              <>
                <div className="number_container dispaly">
                  ...
                  <div className={`round_up ${(isVisible) ? 'visible' : ''}`}><CountUp easingFn={easingFn} end={18.7} decimals={1} duration={4} delay={3} separator="," suffix="% of GDP" useEasing /></div>
                </div>
                <div className="number_container">
                  <h3>Spending gap</h3>
                  <div className={`number number_2 ${(isVisible) ? 'visible' : ''}`}>$286.1 billion</div>
                </div>
                ...
                <div className="dots">
                  {
                    Array(parseInt(5804.7 / 100 / 0.187, 10)).fill().map((el, i) => <div className="dot"><span className={`${i < Math.floor((5804.7 - 286.1) / 100) ? 'current' : i < 5753.7 / 100 ? 'target' : ''}`}>$</span></div>)
                  }
                </div>
